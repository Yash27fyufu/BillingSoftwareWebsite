...
NOTE VARCHAR(200),
  ...
  CASH_PARTY_ID INT NOT NULL PREIMARY KEY,
  NOTE VARCHAR(200),
  ...
  `CREATE TABLE GST_TYPE (
    GST_ID INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
  ...
  `CREATE TABLE HSN_DETAILS (
    HSN_ID INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
  ...
  `CREATE TABLE ITEMS (
    ...
    DISPLAY TINYINT(1) DEFAULT 1,
    ITEM_OPENING_QTY FLOAT DEFAULT NULL,
  ...
  `CREATE TABLE BILL_SERIES (
    BILL_SERIES_ID INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
  ...
  `CREATE TABLE BILL_DETAILS (
    BILL_DETAILS_ID INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
  ...
  `CREATE TABLE BILL_ITEMS (
    BILL_ITEM_ID INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
  ...
  `CREATE TABLE BILL_NOTE (
    BILL_NOTE_ID INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    BILL_DETAILS_ID INT NOT NULL UNIQUE,
    BILL_ID INT NOT NULL,
    BILL_TYPE VARCHAR(20) DEFAULT NULL,
  ...
  PAYMENT_ID INT NOT NULL AUTO_INCREMENT PRIAMRY KEY,
...
FOREIGN KEY (PAYMENT_ID) REFERENCES PAYMENTS (PAYMENT_ID),
  ...
  `CREATE TABLE PAYMENT_ALLOCATION (
    PAYMENT_ALLOCATION_ID INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    PAYMENT_ID INT NOT NULL,
    AMOUNT FLOAT NOT NULL,
    BILL_DETAILS_ID INT NOT NULL,
    BILL_ID INT NOT NULL,
  ...
  APP_USER_NAME VARCHAR(255) DEFAULT NULL,
  APP_USER_NUMBER VARCHAR(15) NOT NULL,
  APP_USER_GST_NUM VARCHAR(20) DEFAULT NULL,
  APP_USER_ADDRESS VARCHAR(255) DEFAULT NULL,
  APP_USER_PINCODE INT DEFAULT NULL,
  APP_USER_MAIL VARCHAR(100) DEFAULT NULL,
  APP_USER_TRANSPORT VARCHAR(50) DEFAULT NULL,
  PRIMARY KEY (APP_USER_ID),
  UNIQUE (APP_USER_NUMBER)
) ENGINE=INNODB DEFAULT CHARSET=UTF8MB4 COLLATE=UTF8MB4_0900_AI_CI;`,

  `CREATE TABLE APP_USER_TOKENS (
  APP_USER_ID INT NOT NULL,
  APP_USER_TOKEN VARCHAR(255) DEFAULT NULL,
  ...
  APP_USER_ID INT NOT NULL,
  ITEM_ID INT NOT NULL,
  PRIMARY KEY (APP_USER_ID,ITEM_ID),
  KEY ITEM_ID (ITEM_ID),
  FOREIGN KEY (APP_USER_ID) REFERENCES APP_USERS (APP_USER_ID),
  FOREIGN KEY (ITEM_ID) REFERENCES ITEMS (ITEM_ID)
) ENGINE=INNODB DEFAULT CHARSET=UTF8MB4 COLLATE=UTF8MB4_0900_AI_CI;`,
  ...
  also run this to avaoid loops in category relations:



                     DELIMITER $$
CREATE PROCEDURE GenerateRandomUntil100(IN PARENT_ID_TEMP INT, IN CHIL_ID_TEMP INT, OUT ANYLOOP INT)
  BEGIN
      DECLARE currentValue INT;
      DECLARE loop_detected BOOLEAN DEFAULT FALSE;

      -- Declare a cursor to select CHILD_ID based on CHIL_ID_TEMP (which is PARENT_ID in the query)
      DECLARE cur CURSOR FOR 
          SELECT CHILD_ID 
          FROM CATEGORY_CATEGORY_RELATION
          WHERE PARENT_ID = CHIL_ID_TEMP;

      -- Declare a handler to set currentValue to NULL when no more rows are found
      DECLARE CONTINUE HANDLER FOR NOT FOUND SET currentValue = NULL;
      ...
      -- Loop through all fetched values until currentValue is NULL (end of data)
      FETCH_LOOP: WHILE currentValue IS NOT NULL DO
          -- Check if the current value matches the PARENT_ID_TEMP
          IF currentValue = PARENT_ID_TEMP THEN
              -- Set ANYLOOP to 1 if a loop is detected and exit the loop
              SET ANYLOOP = 1;
              -- Exit the loop
              LEAVE FETCH_LOOP;
          ELSE
              -- Call the procedure recursively if the value doesn't match
              CALL GenerateRandomUntil100(PARENT_ID_TEMP, currentValue, ANYLOOP);
          END IF;
      ...
      -- If no loop was found, set ANYLOOP to 0
      IF ANYLOOP IS NULL THEN
          SET ANYLOOP = 0;
      END IF;
  ...
  CREATE TRIGGER BeforeInsertTrigger2
  BEFORE INSERT ON CATEGORY_CATEGORY_RELATION
  FOR EACH ROW
  BEGIN
      DECLARE ANYLOOP INT DEFAULT 0;

      -- Call the procedure to check for loops
      CALL GenerateRandomUntil100(NEW.PARENT_ID, NEW.CHILD_ID, ANYLOOP);

      -- If ANYLOOP = 1, abort the insert with an error
      IF ANYLOOP = 1 THEN
          SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'This will create a loop.';
      END IF;
